...
- Use the vendored Luau source and headers as the source of truth for supported syntax and C API behavior. Check signatures, stack effects, and ownership behavior instead of assuming compatibility with stock Lua; for example, Luau's `lua_ref(L, idx)` leaves the referenced value on the stack. Consult the official [Luau syntax](https://luau.org/syntax/) and [C API](https://luau.org/api/) documentation for supplementary guidance.
